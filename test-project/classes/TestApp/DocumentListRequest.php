...
class TestApp_DocumentListRequest extends Adaptor_XMLBase {
	...
	public function  __construct() {
		...
		$this->strHas=new Basictypes_String('');
	}
	/**
	 * Вывод в XMLWriter
	 * @codegen true
	...
	public function toXmlWriter(XMLWriter &$xw,$xmlname=NULL,$xmlns=NULL,$mode=Adaptor_XML::ELEMENT){
		...
		if ($mode&Adaptor_XML::STARTELEMENT) $xw->startElementNS(NULL,$xmlname,$xmlns);
			...
			if($this->strHas!==NULL) {$xw->writeElement("strHas",$this->strHas->LogicalToXSD());}
		...
		while($xr->read()){
			if($xr->nodeType==XMLReader::ELEMENT) {
				...
				switch($xr->localName){
					...
					case "strHas": $this->strHas=$xsinil?NULL:new Basictypes_String($xr->readString(),Adaptor_DataType::XSD); break;
